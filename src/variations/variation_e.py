...
# Nine patches with 1000 preselected permutations
...
model_save_prefix = "variation_2e"
...
class ShufflePatchDataset(Dataset):
  ...
  def __getitem__(self, index):
    ...
    patch_coords = [(y+self.random_jitter(), x+self.random_jitter()) for (y,x) in sub_window_coords]
    
    permutation_index = int(math.floor((permutation_count * random.random())))

    patch_coords = [pc for _,pc in sorted(zip(permutations[permutation_index],patch_coords))]

    patches = [image[y:y+self.patch_dim, x:x+self.patch_dim] for (y,x) in patch_coords]
